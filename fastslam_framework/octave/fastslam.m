...
%for t = 1:50
    ...
    % Perform the prediction step of the particle filter which recives only odometry and updates pose of the robot
    particles = prediction_step(particles, data.timestep(t).odometry, noise);

    % Perform the correction step of the particle filter. 
    %In this stage, each paticle (assuming its pose is correct) constructs a map of its own based on the recived sensor information
